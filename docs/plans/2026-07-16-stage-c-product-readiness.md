...
- [x] Частина 3 — Сторінка /account + видалення акаунта (GDPR) — обрано дешевший
      варіант із двох запропонованих: `_verify_password` імпортовано з
      routers.auth (не виносили в окремий app/security.py). Перевірено наскрізь
      у браузері: невірний пароль → 403 з повідомленням, вірний → 204 → редірект
      на /login, повторний логін тим самим email → 401
- [x] Частина 4 — Landing page + SEO (robots, sitemap, OG) — перевірено наскрізь:
      `/` не редіректить неавторизованого (тарифи підтягуються з бекенда), і
      редіректить залогіненого на /dashboard; /privacy, /terms у робіт/сайтмапі
      посилаються наперед на Частину 5
- [x] Частина 5 — Юридичні сторінки /privacy і /terms — обидві позначені
      HTML-коментарем «ЧЕРНЕТКА — не юридична консультація; власник має
      вичитати перед публічним запуском»; перевірено, що відкриваються без
      логіну (title змінюється, без редіректу на /login)
- [x] Частина 6 — Мобільна адаптація 375px — перевірено наскрізь у браузері
      (375×812): /login, /dashboard, /charts (natal-результат), /persons,
      /account, landing. Знайдено 2 реальні баги: (1) на mobile не було
      навігації взагалі — додано `lg:hidden` рядок NAV-лінків з горизонтальним
      скролом у (dashboard)/layout.tsx (без JS-гамбургера, як і вказано);
      (2) таблиці результатів вилазили за viewport без скролу — обгорнуто в
      `overflow-x-auto` у ResultTables.tsx (Planet/Aspect/ArabicParts) і
      SavedChartsTab.tsx. ВІДХИЛЕННЯ від плану: той самий баг з тим самим
      мінімальним фіксом (тільки Tailwind-обгортка, без зміни логіки)
      застосовано і до TransitForm.tsx та SynastryForm.tsx, хоча "Не робити"
      прямо виключав ці файли — узгоджено з користувачем (AskUserQuestion),
