...
- [x] Частина 1 — Передумови: домен `astrodite.cc` куплено; DNS у adm.tools,
      `api.astrodite.cc`/apex/www вже вказували на `37.60.229.175`.
      **ПОЗАПЛАНОВА знахідка, що коштувала кількох днів:** root-логін по SSH
      на цьому VPS вимкнено (`sshd -T` → `permitrootlogin no`) drop-in
      файлом у `/etc/ssh/sshd_config.d/`, який перекриває `PermitRootLogin
      yes` нижче в головному конфізі — бо `Include` стоїть НА ПОЧАТКУ файлу,
      а OpenSSH бере перше значення. Через це жодне скидання пароля root не
      могло спрацювати в принципі. Вхід знайшовся інший: ключ
      `~/.ssh/hermes_remote_key` від попереднього налаштування Hermes уже
      лежав у `authorized_keys` користувача `deploy`. Йому видано sudo без
      пароля (`/etc/sudoers.d/deploy`, 0440). Ресурси: 7.8 ГБ RAM (стек
      займає ~1 ГБ разом із Hermes), 72 ГБ диска (14%), 4 ядра; порти
      80/443/5432/8000 були вільні — конфлікту з Hermes немає.
      Супутні граблі: `fail2ban` банить нашу IP після кількох невдалих
      SSH-спроб (симптом — `Connection refused`, не таймаут; бан ~10 хв),
      тепер у білому списку `/etc/fail2ban/jail.d/zorya-whitelist.conf`;
      VNC-консоль Contabo губить символи при наборі (зник пробіл у
      `.ssh /root/` і слеші в `/dev/null`) — довгі рядки туди вводити не
      можна. Деталі збережено в пам'яті проєкту.
- [x] Частина 2 — Docker 29.1.3 + Compose 2.40.3 з репозиторіїв Ubuntu
      (замість `curl | sh` — без виконання стороннього скрипта); репо
      склоновано в `/opt/zorya` (гілка `main`); `.env` з правами 600,
      секрети згенеровані `openssl rand` НА СЕРВЕРІ (48 і 64 символи);
      плейсхолдери Stripe очищено, щоб додаток коректно віддавав 503,
      а не намагався працювати з фальшивими ключами.
- [x] Частина 3 — Перший запуск: db+api піднято, усі 8 міграцій застосовано
      на PostgreSQL (включно з `unique payment provider ids` і
      `chart quotas`), атлас імпортовано — 34 099 міст. Caddy видав
      сертифікат Let's Encrypt для `api.astrodite.cc` (tls-alpn-01),
      HTTP→HTTPS 308, перевірено ззовні: `/health` → ok, пошук «Kyiv» →
      Europe/Kyiv. **Знайдено й виправлено ваду власного конфігу:** сервісу
      `caddy` у `docker-compose.yml` не передавався `API_DOMAIN` (ні
      `env_file`, ні `environment`), тож Caddyfile мовчки падав на дефолт
      `api.localhost` і випускав самопідписаний локальний сертифікат
      замість справжнього — фікс у коміті `f637ca7`.
      Побічно: під час ротації пароля БД скрипт обірвався між `ALTER USER`
      і записом у `.env` (через `set -e`), лишивши їх розсинхронізованими —
      полагоджено, взявши за істину `.env`; врятувало те, що Postgres
      приймає локальні підключення через сокет без пароля.
- [x] Частина 4 — Vercel: проєкт `astrology-web-app-1` імпортовано з GitHub
      (Root Directory `apps/frontend`, preset Next.js — спершу стояв `Other`,
      виправлено до деплою), env `API_URL=https://api.astrodite.cc`.
      Домен `astrodite.cc` привʼязано як канонічний (галочку «redirect apex
      to www» свідомо знято, щоб адреса збігалася з `FRONTEND_URL` у
      бекенді — саме вона підставляється в листи підтвердження/скидання
      пароля й у редірект після оплати monobank).
      **ГРАБЛІ, що поклали бекенд на кілька хвилин:** у DNS було створено
      запис-шаблон `*.astrodite.cc` → Vercel, і він перехопив
      `api.astrodite.cc` (окремий A-запис для `api` при цьому зник) —
      `https://api.astrodite.cc` перестав відповідати. Полагоджено:
      повернуто явний `api` → `37.60.229.175`, шаблон видалено. Урок:
      для Vercel потрібні лише корінь і `www`, шаблон — пастка.
      Наскрізна перевірка в проді (на `*.vercel.app`, до перемикання
      домену): реєстрація → дашборд → розрахунок натальної карти з
      колесом, таблицею планет українською та аспектами; нова таблиця
      `chart_quotas` відпрацювала (`used=1`). Тестовий акаунт і всі його
      рядки видалено з продакшн-бази. Security-заголовки на місці, HSTS
      Vercel додає сам (закриває питання, залишене відкритим у C3).
      Лишилось: `www.astrodite.cc` треба додати у Vercel як редірект на
      корінь — DNS на нього вказує, але сертифіката без цього немає.
- [ ] Частина 5 — Live-ключі: Stripe, monopay, пошта, Sentry, Plausible, feedback
- [~] Частина 6 — Бекапи ГОТОВО: `scripts/backup_db.sh` у crontab root
      (щодня 3:00, ротація 14 днів, лог `/var/log/zorya-backup.log`);
      перший дамп 877 КБ. **Репетицію відновлення проведено** (плану це
      вимагав): дамп розгорнуто в тимчасову БД `zorya_restore_test` —
      34 099 міст і всі 12 таблиць на місці, збігається з продом; тимчасову
      базу видалено, прод не чіпався. `www.astrodite.cc` → 307 на корінь,
      сертифікат випущено. ЛИШИЛОСЬ: uptime-моніторинг (UptimeRobot — дія
      власника), перевірка автопідйому стека після ребуту (ребут відкладено,
      бо перезапустить і Hermes), live-ключі (Частина 5).
- [ ] DoD — див. внизу
