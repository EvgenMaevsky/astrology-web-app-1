...
## Definition of Done
1. `pytest -q` — 0 failed; жодного liqpay-тесту, всі 9 monopay-сценаріїв є.
2. `tsc --noEmit && next build` — чисто.
3. Міграція застосовується на чистій БД; `monopay_invoice_id` у схемі.
4. `grep -ri liqpay` по `apps/frontend/app` і `services/astro-api/app` +
   `tests` — нуль збігів (docs/README/історія — дозволено).
5. Живий цикл з тестовим токеном: оплата → Pro з period_end → cancel →
   reversed → free (або чесно задокументоване blocked, якщо токена немає).
6. Тексти pricing/terms/privacy кажуть правду про «30 днів без
   автопродовження»; ROADMAP оновлено.
